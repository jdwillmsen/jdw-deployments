...
{{- end -}}
{{- /* Every bot's PRESENCE_URL names the agent's metrics Service, and the
agent Deployment behind it renders only with the console bridge too: without
it the bots would poll a Service with no pod and never leave the state they
last saw, so a parked bot would stay parked. */ -}}
{{- if and .Values.global.presence.enabled (not (and .Values.agent.enabled .Values.global.consoleBridge.enabled)) -}}
{{- fail "global.presence.enabled needs the agent Deployment, which renders only with both agent.enabled and global.consoleBridge.enabled: turning either off, the bridge included, means turning presence off in the same change" -}}
{{- end -}}
{{- if ne $agents 1 -}}
